...
public:
	...
	// 頭
	static constexpr float COLLISION_RADIUS_HEAD = 30.0f;
	// 体
	static constexpr float COLLISION_RADIUS_BODY = 35.0f;
	// 腕
	static constexpr float COLLISION_RADIUS_ARM = 10.0f;
	// 手
	...
	// モデルと体の相対座標(上)
	static constexpr float OFFSET_POS_BODY_TOP = -40.0f;
	// モデルと体の相対座標(下)
	static constexpr float OFFSET_POS_BODY_UNDER = 15.0f;

	// モデルと腕の相対座標(上)
	static constexpr float OFFSET_POS_ARM_TOP = -2.0f;
	// モデルと腕の相対座標(下)
	static constexpr float OFFSET_POS_ARM_UNDER = -10.0f;

	// モデルと手の相対座標
	static constexpr float OFFSET_POS_HAND = 5.0f;

	// 攻撃可能範囲
	static constexpr float ATTACK_RANGE = 95.0f;

	// 攻撃待ち時間
	static constexpr float ATTACK_COOLDOWN = 3.0f;

	Zombie(ENEMY_TYPE type, int baseModelId, Player* player);
